    ...
    <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
        ...
        <div class="table-responsive">
            <div class="jumbotron">
                ...
                <p>{{$order->adresse->adresse}}<br>
                    {{$order->adresse->adresse2}}<br>
                    {{$order->adresse->code_postale}} - {{$order->adresse->ville}} - {{$order->adresse->pays}}</p>
{{--                <p>Numéro de transaction Stripe: </p>--}}
            ...
            <table class="table table-bordered">
                ...
                <tr>
                    <td>{{$product->id}}</td>
                    <td>
                        <img width="50" class="rounded-circle img-thumbnail" src="produits/{{$product->photo_principale}}" alt="">
                        <strong>{{$product->nom}} {{strtoupper($product->pivot->size)}} </strong> <br>
                    </td>
